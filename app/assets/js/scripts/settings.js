...
// Microsoft constants removed for patcher mode - Creating stubs to avoid errors
const MSFT_OPCODE = { REPLY_LOGIN: '', REPLY_LOGOUT: '', OPEN_LOGOUT: '' }
const MSFT_REPLY_TYPE = { ERROR: '', SUCCESS: '' }
const MSFT_ERROR = { NOT_FINISHED: '' }
...
function fullSettingsSave() {
    ...
    // Sauvegarder également les paramètres Metin2 dans le fichier de config
    try {
        const width = ConfigManager.getGameWidth()
        const height = ConfigManager.getGameHeight()
        const fullscreen = ConfigManager.getFullscreen()
        const musicVolume = ConfigManager.getMusicVolume()
        
        // Convertir le pourcentage (0-100) en valeur décimale (0.000-1.000)
        const volumeDecimal = (musicVolume / 100).toFixed(3)
        
        // Récupérer la qualité graphique actuelle depuis le sélecteur
        const graphicsQualitySelect = document.getElementById('metin2GraphicsQuality')
        let graphicsQualitySettings = {}
        
        if (graphicsQualitySelect) {
            const quality = graphicsQualitySelect.value
            console.log('Sauvegarde de la qualité graphique:', quality)
            
            // Calculer les paramètres selon la qualité
            switch (quality) {
                case 'low':
                    graphicsQualitySettings = {
                        SHADOW_TARGET_LEVEL: 1,
                ...
                case 'medium':
                    graphicsQualitySettings = {
                        ...
                        SHADOW_QUALITY_LEVEL: 1,
                        DROP_EFFECT: 2
                    }
                    break
                case 'high':
                    graphicsQualitySettings = {
                        SHADOW_TARGET_LEVEL: 3,
                        SHADOW_QUALITY_LEVEL: 2,
                        DROP_EFFECT: 4
                    }
                    break
            }
        ...
        saveToMetin2Config({
            ...
            MUSIC_VOLUME: volumeDecimal,
            ...graphicsQualitySettings
        })
        
        console.log('Paramètres Metin2 sauvegardés avec succès')
    } catch (error) {
        console.error('Erreur lors de la sauvegarde des paramètres Metin2:', error)
    }
}
...
// Account buttons disabled for patcher mode
document.getElementById('settingsAddMojangAccount').onclick = (e) => {
    // Disabled for patcher mode
    console.log('Account management disabled in patcher mode')
}
...
// Microsoft login handlers disabled for patcher mode
/*
...
function populateAuthAccounts(){
    // Simplified for patcher mode - no accounts needed
    return
...
function prepareAccountsTab() {
    // Simplified for patcher mode - skip account population
    console.log('Account tab preparation skipped in patcher mode')
}
...
async function prepareSettings(first = false) {
    try {
        console.log('Préparation des paramètres pour Metin2...')
        ...
        // Initialiser les paramètres Metin2
        initializeMetin2Settings()
        
        console.log('Paramètres prêts pour Metin2')
    } catch (error) {
        console.error('Erreur lors de la préparation des paramètres:', error)
        // Continue même en cas d'erreur pour ne pas bloquer l'interface
    }
}
...
/*******************************************************************************
 *                                                                             *
 * Account Import Modal Functionality                                          *
 *                                                                             *
 ******************************************************************************/

// Variables pour la modal d'import
let importAccountsModal = null
let selectedImportFolder = null

/**
 * Initialise la fonctionnalité d'import des comptes
 */
function initializeAccountImport() {
    // Récupérer les éléments DOM
    const importButton = document.getElementById('settingsImportAccountsButton')
    importAccountsModal = document.getElementById('importAccountsModal')
    ...
    const selectFolderButton = document.getElementById('selectImportFolderButton')
    ...
    if (importButton) {
        importButton.addEventListener('click', () => {
            openImportAccountsModal()
        })
    }

    // Fermer la modal
    const closeModal = () => {
        importAccountsModal.style.display = 'none'
        resetImportModal()
    }
    ...
    if (importAccountsModal) {
        importAccountsModal.addEventListener('click', (e) => {
            if (e.target === importAccountsModal) {
                closeModal()
            }
        })
    }
    ...
    if (confirmButton) {
        confirmButton.addEventListener('click', async () => {
            await confirmAccountImport()
        })
    }
}

/**
 * Ouvre la modal d'import des comptes
 */
function openImportAccountsModal() {
    if (importAccountsModal) {
        importAccountsModal.style.display = 'flex'
        resetImportModal()
    }
}

/**
 * Remet à zéro l'état de la modal
 */
function resetImportModal() {
    ...
    if (folderPathInput) {
        ...
        folderPathInput.placeholder = 'Aucun dossier sélectionné'
    }
...
/**
 * Sélectionne le dossier contenant les comptes à importer
 */
async function selectImportFolder() {
    try {
        ...
        const result = await dialog.showOpenDialog({
            title: 'Sélectionner le dossier contenant les comptes',
            properties: ['openDirectory'],
            buttonLabel: 'Sélectionner'
        })
        
        if (!result.canceled && result.filePaths.length > 0) {
            selectedImportFolder = result.filePaths[0]
            const folderPathInput = document.getElementById('importAccountsFolderPath')
            const confirmButton = document.getElementById('importAccountsConfirm')
            
            if (folderPathInput) {
                folderPathInput.value = selectedImportFolder
            }
            ...
            console.log('Dossier sélectionné:', selectedImportFolder)
        }
    } catch (error) {
        console.error('Erreur lors de la sélection du dossier:', error)
        showImportStatus('error', 'Erreur lors de la sélection du dossier')
    }
}

/**
 * Confirme l'import des comptes
 */
async function confirmAccountImport() {
    ...
    try {
        ...
        showImportStatus('loading', 'Recherche des comptes...')
        
        // Simuler l'import (ici tu peux ajouter la vraie logique d'import)
        await simulateAccountImport()
        
        showImportStatus('success', 'Comptes importés avec succès!')
        ...
        showImportStatus('error', `Erreur lors de l'import: ${error.message}`)
        
        if (confirmButton) {
            confirmButton.disabled = false
            confirmButton.textContent = 'Importer'
        }
    }
}

/**
 * Simule l'import des comptes (à remplacer par la vraie logique)
 */
async function simulateAccountImport() {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            // Ici tu peux ajouter la vraie logique d'import
            // Par exemple, lire les fichiers de comptes dans le dossier sélectionné
            console.log('Import simulé depuis:', selectedImportFolder)
    ...
    if (statusIcon && statusText) {
        ...
        switch (type) {
            case 'loading':
                ...
                statusIcon.style.animation = 'rotating 2s linear infinite'
                break
            case 'success':
                statusIcon.textContent = '✓'
...
// Initialiser la fonctionnalité d'import au chargement
document.addEventListener('DOMContentLoaded', () => {
    initializeAccountImport()
})

// Export functions globally so they can be used in other scripts
window.prepareSettings = prepareSettings
window.prepareAccountsTab = prepareAccountsTab
window.initializeAccountImport = initializeAccountImport
...
/**
 * Initialise la navigation des onglets des paramètres
 */
function initializeSettingsNavigation() {
    ...
    console.log('Boutons de navigation trouvés:', navItems.length)
    
    navItems.forEach((item, index) => {
        console.log(`Bouton ${index}:`, item.textContent, item.getAttribute('rSc'))
        
        item.addEventListener('click', (e) => {
            console.log('Clic détecté sur:', item.textContent)
            e.preventDefault()
            const targetTab = item.getAttribute('rSc')
            
            if (targetTab) {
                console.log('Changement d\'onglet vers:', targetTab)
                switchSettingsTab(targetTab)
            } else if (item.id === 'settingsNavDone') {
                console.log('Fermeture des paramètres')
                closeSettings()
            } else {
                console.log('Aucune action définie pour:', item.textContent)
            }
        })
    })
    
    // Vérifier le bouton Done séparément
    const doneButton = document.getElementById('settingsNavDone')
    if (doneButton) {
        console.log('Bouton Done trouvé')
        doneButton.addEventListener('click', (e) => {
            console.log('Clic sur Done')
            e.preventDefault()
            closeSettings()
        })
    } else {
        console.log('Bouton Done non trouvé')
    }
}

/**
 * Bascule vers un onglet spécifique des paramètres
 */
function switchSettingsTab(targetTabId) {
    // Masquer tous les onglets
    const allTabs = document.querySelectorAll('.settingsTab')
    allTabs.forEach(tab => {
        tab.style.display = 'none'
    })
    
    // Désélectionner tous les boutons de navigation
    const allNavItems = document.querySelectorAll('.settingsNavItem')
    ...
    if (targetTab) {
        ...
        console.log('Onglet affiché:', targetTabId)
        
        // Si c'est l'onglet Metin2, initialiser les paramètres
        if (targetTabId === 'settingsTabMetin2') {
            onMetin2TabSelected()
        }
    }
    
    // Sélectionner le bouton de navigation correspondant
    const targetNavItem = document.querySelector(`[rSc="${targetTabId}"]`)
    if (targetNavItem) {
        targetNavItem.setAttribute('selected', '')
        targetNavItem.classList.add('selected')
        console.log('Bouton sélectionné:', targetTabId)
    }
}

/**
 * Ferme les paramètres et retourne au launcher principal
 */
...
// Aussi initialiser quand la fenêtre est chargée (au cas où)
...
/*******************************************************************************
 *                                                                             *
 * Metin2 Settings Functionality                                               *
 *                                                                             *
 ******************************************************************************/

/**
 * Initialise la fonctionnalité des paramètres Metin2
 */
function initializeMetin2Settings() {
    console.log('Initialisation des paramètres Metin2...')
    
    // S'assurer que le système de sélecteurs existant inclut notre nouveau sélecteur
    rebindSettingsSelectors()
    
    // Initialiser les sélecteurs de résolution
    ...
    bindMetin2SettingsListeners()
    
    // Initialiser les boutons d'action
    initializeMetin2ActionButtons()
    
    // Initialiser les boutons d'action des comptes
    initializeAccountActionButtons()
    
    // Initialiser le sélecteur de qualité graphique
    initializeGraphicsQualitySelector()
}

/**
 * Re-bind les sélecteurs pour inclure les nouveaux éléments
 */
function rebindSettingsSelectors() {
    // Re-exécuter bindSettingsSelect pour inclure notre nouveau sélecteur
    for(let ele of document.getElementsByClassName('settingsSelectContainer')) {
        const selectedDiv = ele.getElementsByClassName('settingsSelectSelected')[0]
...
function initializeResolutionSelector() {
    const resolutionSelected = document.getElementById('metin2ResolutionSelected')
    const resolutionOptions = document.getElementById('metin2ResolutionOptions')
    const customContainer = document.getElementById('customResolutionContainer')
    
    if (!resolutionSelected || !resolutionOptions) {
        console.log('Éléments de résolution non trouvés')
        return
    }
    
    console.log('Initialisation du sélecteur de résolution...')
    console.log('Selected element:', resolutionSelected)
    console.log('Options element:', resolutionOptions)
    
    // Gérer la sélection d'une option avec la logique simplifiée
    const options = resolutionOptions.querySelectorAll('div[value]')
    console.log('Options trouvées:', options.length)
    
    options.forEach((option, index) => {
        console.log(`Option ${index}:`, option.getAttribute('value'), option.textContent)
        
        // Supprimer les anciens listeners
        option.removeEventListener('click', handleResolutionClick)
        
        // Ajouter le nouveau listener
        option.addEventListener('click', function(e) {
            console.log('=== CLIC SUR OPTION ===')
            console.log('Option cliquée:', this.getAttribute('value'))
            console.log('Texte option:', this.innerHTML)
            
            const value = this.getAttribute('value')
            const text = this.innerHTML
            
            // Mise à jour du texte sélectionné
            console.log('Ancien texte:', resolutionSelected.innerHTML)
            resolutionSelected.innerHTML = text
            console.log('Nouveau texte:', resolutionSelected.innerHTML)
            ...
            // Fermer le dropdown
            resolutionOptions.setAttribute('hidden', '')
            resolutionSelected.classList.remove('select-arrow-active')
            
            // Gérer l'affichage du conteneur personnalisé
            handleResolutionChange(value, customContainer)
            
            console.log('=== FIN CLIC ===')
        })
    })
    
    console.log('Sélecteur de résolution initialisé')
}

/**
 * Gère le changement de résolution
 */
function handleResolutionChange(value, customContainer) {
    console.log('Gestion du changement de résolution:', value)
    
    if (!customContainer) {
        customContainer = document.getElementById('customResolutionContainer')
    }
    
    if (customContainer) {
        if (value === 'custom') {
            customContainer.style.display = 'block'
            console.log('Affichage du conteneur personnalisé')
        } else {
            customContainer.style.display = 'none'
            
            // Appliquer la résolution prédéfinie
            if (value && value.includes('x')) {
                const [width, height] = value.split('x')
                const widthInput = document.getElementById('metin2CustomWidth')
                const heightInput = document.getElementById('metin2CustomHeight')
                
                console.log('Application de la résolution:', width, 'x', height)
                ...
                if (widthInput && heightInput) {
                    ...
                    saveMetin2Resolution(parseInt(width), parseInt(height))
                }
            }
        }
    }
...
/**
 * Handler pour les clics sur les options (pour pouvoir le supprimer)
 */
function handleResolutionClick(e) {
    // Cette fonction sera remplacée par les listeners individuels
}

/**
 * Initialise les boutons d'action Metin2
 */
function initializeMetin2ActionButtons() {
    console.log('Initialisation des boutons d\'action Metin2...')
    
    // Bouton pour ouvrir le dossier du jeu
    const gameFolderButton = document.getElementById('metin2OpenGameFolder')
    if (gameFolderButton) {
        gameFolderButton.addEventListener('click', openGameFolder)
        console.log('Bouton dossier du jeu initialisé')
    } else {
        console.log('Bouton dossier du jeu non trouvé')
    }
    
    // Bouton pour ouvrir le dossier de configuration
    const configFolderButton = document.getElementById('metin2OpenConfigFolder')
    if (configFolderButton) {
        configFolderButton.addEventListener('click', openConfigFolder)
        console.log('Bouton dossier config initialisé')
    } else {
        console.log('Bouton dossier config non trouvé')
    }
}

/**
 * Ouvre le dossier du jeu Metin2
 */
function openGameFolder() {
    try {
        console.log('Ouverture du dossier du jeu...')
        
        const { shell } = require('@electron/remote')
        const path = require('path')
        const fs = require('fs')
        
        // Essayer de trouver le dossier du jeu
        // D'abord, essayer le dossier courant du launcher
        const currentDir = process.cwd()
        console.log('Dossier courant:', currentDir)
        
        // Vérifier si on est déjà dans le dossier du jeu (chercher des fichiers typiques de Metin2)
        const possibleGameFiles = ['metin2client.exe', 'Metin2Client.exe', 'client.exe']
        let gameFolder = null
        
        // Vérifier dans le dossier courant
        for (const file of possibleGameFiles) {
            const filePath = path.join(currentDir, file)
            if (fs.existsSync(filePath)) {
                gameFolder = currentDir
                break
            }
        }
        
        // Si pas trouvé, essayer le dossier parent
        if (!gameFolder) {
            const parentDir = path.dirname(currentDir)
            for (const file of possibleGameFiles) {
                const filePath = path.join(parentDir, file)
                if (fs.existsSync(filePath)) {
                    gameFolder = parentDir
                    break
                }
            }
        }
        
        // Si toujours pas trouvé, utiliser le dossier courant par défaut
        if (!gameFolder) {
            gameFolder = currentDir
            console.log('Dossier du jeu non détecté automatiquement, utilisation du dossier courant')
        }
        
        console.log('Ouverture du dossier:', gameFolder)
        shell.openPath(gameFolder)
        
        // Afficher une notification de succès
        showNotification('Dossier du jeu ouvert avec succès', 'success')
        
    } catch (error) {
        console.error('Erreur lors de l\'ouverture du dossier du jeu:', error)
        showNotification('Erreur lors de l\'ouverture du dossier du jeu', 'error')
    }
}

/**
 * Ouvre le dossier de configuration Metin2
 */
function openConfigFolder() {
    try {
        console.log('Ouverture du dossier de configuration...')
        ...
        const configFolder = path.join(os.homedir(), 'Documents', GAME_FOLDER_NAME)
        console.log('Dossier de configuration:', configFolder)
        
        // Créer le dossier s'il n'existe pas
        if (!fs.existsSync(configFolder)) {
            fs.mkdirSync(configFolder, { recursive: true })
            console.log('Dossier de configuration créé')
        }
        
        shell.openPath(configFolder)
        console.log('Dossier de configuration ouvert')
        
        // Afficher une notification de succès
        showNotification('Dossier de configuration ouvert avec succès', 'success')
        
    } catch (error) {
        console.error('Erreur lors de l\'ouverture du dossier de configuration:', error)
        showNotification('Erreur lors de l\'ouverture du dossier de configuration', 'error')
    }
}
...
function showNotification(message, type = 'info') {
    ...
    // Ici on pourrait ajouter une vraie notification UI si nécessaire
    // Pour l'instant, on utilise juste la console
}

/**
 * Initialise les boutons d'action des comptes
 */
function initializeAccountActionButtons() {
    console.log('Initialisation des boutons d\'action des comptes...')
    
    // Bouton pour ajouter un compte
    const addAccountButton = document.getElementById('settingsAddAccountButton')
    if (addAccountButton) {
        addAccountButton.addEventListener('click', addAccount)
        console.log('Bouton ajouter compte initialisé')
    } else {
        console.log('Bouton ajouter compte non trouvé')
    }
}

/**
 * Gère l'ajout d'un nouveau compte
 */
function addAccount() {
    try {
        ...
        showNotification('Fonctionnalité d\'ajout de compte à implémenter', 'info')
        
        // Exemple de ce qui pourrait être fait :
        // - Ouvrir une modal de connexion
        // - Rediriger vers une page d'authentification
        // - Intégrer avec un système d'auth existant
...
/**
 * Initialise le sélecteur de qualité graphique
 */
function initializeGraphicsQualitySelector() {
    console.log('Initialisation du sélecteur de qualité graphique...')
    
    const graphicsQualitySelect = document.getElementById('metin2GraphicsQuality')
    if (!graphicsQualitySelect) {
        console.log('Sélecteur de qualité graphique non trouvé')
        return
    }
    
    // Charger la qualité actuelle
    const currentQuality = ConfigManager.getGraphicsQuality ? ConfigManager.getGraphicsQuality() : 'medium'
    console.log('Qualité graphique actuelle:', currentQuality)
    
    // Définir la valeur actuelle
    graphicsQualitySelect.value = currentQuality
    
    // Ajouter le listener pour les changements
    graphicsQualitySelect.addEventListener('change', (e) => {
        const selectedQuality = e.target.value
        console.log('Changement de qualité graphique vers:', selectedQuality)
        saveGraphicsQualitySettings(selectedQuality)
    })
    
    console.log('Sélecteur de qualité graphique initialisé')
}

/**
 * Sauvegarde les paramètres de qualité graphique
 */
function saveGraphicsQualitySettings(quality) {
    try {
        console.log('Sauvegarde des paramètres de qualité graphique:', quality)
        
        // Définir les valeurs selon la qualité choisie
        let shadowTargetLevel, shadowQualityLevel, dropEffect
        ...
        switch (quality) {
            ...
            case 'high':
                ...
                break
            default:
                console.warn('Qualité graphique inconnue:', quality)
                return
        }
        
        console.log('Paramètres calculés:', { shadowTargetLevel, shadowQualityLevel, dropEffect })
        
        // Sauvegarder dans la configuration du launcher si la fonction existe
        if (ConfigManager.setGraphicsQuality) {
            ConfigManager.setGraphicsQuality(quality)
            ConfigManager.save()
        }
        
        // Sauvegarder dans le fichier metin2.cfg
        saveToMetin2Config({
            SHADOW_TARGET_LEVEL: shadowTargetLevel,
            SHADOW_QUALITY_LEVEL: shadowQualityLevel,
            DROP_EFFECT: dropEffect
        })
        
        console.log('Paramètres de qualité graphique sauvegardés avec succès')
        showNotification(`Qualité graphique définie sur: ${getQualityLabel(quality)}`, 'success')
        
    } catch (error) {
        console.error('Erreur lors de la sauvegarde des paramètres de qualité graphique:', error)
        showNotification('Erreur lors de la sauvegarde de la qualité graphique', 'error')
    }
}

/**
 * Retourne le libellé de la qualité graphique
 */
...
/**
 * Charge les paramètres Metin2 actuels
 */
function loadMetin2Settings() {
    try {
        // Charger la résolution actuelle
        const currentWidth = ConfigManager.getGameWidth()
        const currentHeight = ConfigManager.getGameHeight()
        const currentFullscreen = ConfigManager.getFullscreen()
        const currentMusicVolume = ConfigManager.getMusicVolume()
        
        console.log('Paramètres actuels:', { width: currentWidth, height: currentHeight, fullscreen: currentFullscreen, musicVolume: currentMusicVolume })
        
        // Mettre à jour les champs de résolution
        const widthInput = document.getElementById('metin2CustomWidth')
        const heightInput = document.getElementById('metin2CustomHeight')
        const fullscreenCheckbox = document.getElementById('metin2Fullscreen')
        ...
        if (fullscreenCheckbox) fullscreenCheckbox.checked = currentFullscreen
        if (musicVolumeSlider) {
            musicVolumeSlider.value = currentMusicVolume
            if (musicVolumeValue) musicVolumeValue.textContent = currentMusicVolume + '%'
        }
        
        // Mettre à jour le sélecteur de résolution
        updateResolutionSelector(currentWidth, currentHeight)
        
        // Charger et mettre à jour la qualité graphique
        loadGraphicsQualityFromConfig()
        
    } catch (error) {
        console.error('Erreur lors du chargement des paramètres Metin2:', error)
    }
}

/**
 * Charge la qualité graphique depuis le fichier metin2.cfg
 */
function loadGraphicsQualityFromConfig() {
    try {
        const fs = require('fs')
        const path = require('path')
        const os = require('os')
        
        // Chemin vers le fichier metin2.cfg
        const documentsPath = path.join(os.homedir(), 'Documents', GAME_FOLDER_NAME)
        const configPath = path.join(documentsPath, 'metin2.cfg')
        
        if (!fs.existsSync(configPath)) {
            console.log('Fichier metin2.cfg non trouvé, utilisation des valeurs par défaut')
            return
        }
        ...
        const shadowTargetLevel = extractConfigValue(configContent, 'SHADOW_TARGET_LEVEL', '2')
        const shadowQualityLevel = extractConfigValue(configContent, 'SHADOW_QUALITY_LEVEL', '1')
        const dropEffect = extractConfigValue(configContent, 'DROP_EFFECT', '2')
        
        console.log('Valeurs actuelles du fichier config:', { shadowTargetLevel, shadowQualityLevel, dropEffect })
        
        // Déterminer la qualité graphique basée sur ces valeurs
        const quality = determineGraphicsQuality(parseInt(shadowTargetLevel), parseInt(shadowQualityLevel), parseInt(dropEffect))
        console.log('Qualité graphique déterminée:', quality)
        
        // Mettre à jour le sélecteur
        const graphicsQualitySelect = document.getElementById('metin2GraphicsQuality')
        if (graphicsQualitySelect) {
            graphicsQualitySelect.value = quality
        }
        
        // Sauvegarder dans ConfigManager si la fonction existe
        if (ConfigManager.setGraphicsQuality) {
            ConfigManager.setGraphicsQuality(quality)
        }
        
    } catch (error) {
        console.error('Erreur lors du chargement de la qualité graphique:', error)
    }
}

/**
 * Extrait une valeur du contenu du fichier config
 */
function extractConfigValue(content, key, defaultValue) {
...
/**
 * Détermine la qualité graphique basée sur les valeurs du fichier config
 */
function determineGraphicsQuality(shadowTargetLevel, shadowQualityLevel, dropEffect) {
    // Vérifier les combinaisons exactes
    if (shadowTargetLevel === 1 && shadowQualityLevel === 0 && dropEffect === 0) {
        return 'low'
    } else if (shadowTargetLevel === 2 && shadowQualityLevel === 1 && dropEffect === 2) {
        return 'medium'
    } else if (shadowTargetLevel === 3 && shadowQualityLevel === 2 && dropEffect === 4) {
        return 'high'
    }
    
    // Si aucune correspondance exacte, essayer de déterminer la qualité la plus proche
    if (shadowTargetLevel <= 1 && shadowQualityLevel <= 0 && dropEffect <= 0) {
        return 'low'
    } else if (shadowTargetLevel <= 2 && shadowQualityLevel <= 1 && dropEffect <= 2) {
        return 'medium'
...
/**
 * Met à jour le sélecteur de résolution basé sur les dimensions actuelles
 */
function updateResolutionSelector(width, height) {
    const resolutionSelected = document.getElementById('metin2ResolutionSelected')
    const resolutionOptions = document.getElementById('metin2ResolutionOptions')
    const customContainer = document.getElementById('customResolutionContainer')
    
    if (!resolutionSelected || !resolutionOptions) return
    ...
    options.forEach(option => {
        ...
        if (option.getAttribute('value') === resolutionString) {
    ...
    // Si aucune correspondance, utiliser "Personnalisée"
    if (!foundMatch) {
        const customOption = resolutionOptions.querySelector('div[value="custom"]')
        if (customOption) {
            customOption.setAttribute('selected', '')
            resolutionSelected.textContent = 'Personnalisée'
            customContainer.style.display = 'block'
        }
    }
}
...
/**
 * Ajoute les listeners pour les paramètres Metin2
 */
function bindMetin2SettingsListeners() {
    // Listener pour les champs de résolution personnalisée
    const widthInput = document.getElementById('metin2CustomWidth')
    const heightInput = document.getElementById('metin2CustomHeight')
    const fullscreenCheckbox = document.getElementById('metin2Fullscreen')
    const musicVolumeSlider = document.getElementById('metin2MusicVolume')
    ...
    if (widthInput) {
        widthInput.addEventListener('input', debounce(() => {
            ...
            if (width && height) {
                saveMetin2Resolution(width, height)
            }
        }, 500))
    }
    ...
    if (fullscreenCheckbox) {
        fullscreenCheckbox.addEventListener('change', () => {
            saveMetin2Fullscreen(fullscreenCheckbox.checked)
        })
    }
    ...
    if (musicVolumeSlider) {
        musicVolumeSlider.addEventListener('input', () => {
            ...
            saveMetin2MusicVolume(volume)
        })
    }
}

/**
 * Sauvegarde la résolution Metin2
 */
function saveMetin2Resolution(width, height) {
    try {
        console.log('Sauvegarde de la résolution:', width, 'x', height)
        
        // Sauvegarder dans la configuration du launcher
        ConfigManager.setGameWidth(width)
        ConfigManager.setGameHeight(height)
        ConfigManager.save()
        
        // Sauvegarder dans le fichier metin2.cfg
        saveToMetin2Config({ WIDTH: width, HEIGHT: height })
        
        console.log('Résolution sauvegardée avec succès')
    } catch (error) {
        console.error('Erreur lors de la sauvegarde de la résolution:', error)
    }
}

/**
 * Sauvegarde le mode plein écran Metin2
 */
function saveMetin2Fullscreen(fullscreen) {
    try {
        console.log('Sauvegarde du mode plein écran:', fullscreen)
        
        // Sauvegarder dans la configuration du launcher
        ConfigManager.setFullscreen(fullscreen)
        ConfigManager.save()
        
        // Sauvegarder dans le fichier metin2.cfg (WINDOWED est l'inverse de fullscreen)
        saveToMetin2Config({ WINDOWED: fullscreen ? 0 : 1 })
        
        console.log('Mode plein écran sauvegardé avec succès')
    } catch (error) {
        console.error('Erreur lors de la sauvegarde du mode plein écran:', error)
    }
}

/**
 * Sauvegarde le volume de la musique Metin2
 */
function saveMetin2MusicVolume(volume) {
    try {
        console.log('Sauvegarde du volume de la musique:', volume)
        ...
        // Convertir le pourcentage (0-100) en valeur décimale (0.000-1.000)
        const volumeDecimal = (volume / 100).toFixed(3)
        ...
        console.log('Volume de la musique sauvegardé avec succès:', volumeDecimal)
        
    } catch (error) {
        console.error('Erreur lors de la sauvegarde du volume de la musique:', error)
    }
}

/**
 * Sauvegarde les paramètres dans le fichier metin2.cfg
 */
function saveToMetin2Config(settings) {
    try {
        const fs = require('fs')
        const path = require('path')
        const os = require('os')
        
        // Chemin vers le fichier metin2.cfg dans Documents/Ascend2
        const documentsPath = path.join(os.homedir(), 'Documents', GAME_FOLDER_NAME)
        const configPath = path.join(documentsPath, 'metin2.cfg')
        
        console.log('Chemin du fichier config:', configPath)
        
        // Créer le dossier s'il n'existe pas
        if (!fs.existsSync(documentsPath)) {
            fs.mkdirSync(documentsPath, { recursive: true })
            console.log('Dossier Ascend2 créé dans Documents')
        }
        ...
        if (fs.existsSync(configPath)) {
            ...
            // Nettoyer les lignes vides supplémentaires à la fin
            configContent = configContent.trimEnd()
        } else {
            // Créer un fichier de base avec les valeurs par défaut
            configContent = `WIDTH						1280
HEIGHT						720
BPP						32
FREQUENCY					60
SOFTWARE_CURSOR			0
...
USE_DEFAULT_IME		0
SOFTWARE_TILING		0
SHADOW_TARGET_LEVEL		4
SHADOW_QUALITY_LEVEL		0
FOV						47.67
DOG_MODE_STATUS			0
RAIN_LEVEL					0
STONE_SCALE			0.213115`
        }
        ...
        Object.keys(settings).forEach(key => {
            ...
            if (regex.test(configContent)) {
                ...
                // Ajouter la nouvelle valeur à la fin sans ligne vide supplémentaire
                // Supprimer les espaces/lignes vides à la fin du contenu existant
                configContent = configContent.trimEnd()
                configContent += `\n${key}\t\t\t\t\t\t${value}`
            }
        })
        ...
        console.log('Fichier metin2.cfg mis à jour avec succès')
        
    } catch (error) {
        console.error('Erreur lors de la sauvegarde dans metin2.cfg:', error)
    }
}
...
// Initialiser les paramètres Metin2 au chargement
document.addEventListener('DOMContentLoaded', () => {
...
// Aussi initialiser quand on change d'onglet vers Metin2
function onMetin2TabSelected() {
    console.log('Onglet Metin2 sélectionné')
    setTimeout(() => {
        initializeMetin2Settings()
    }, 100)
}
...
window.loadMetin2Settings = loadMetin2Settings
window.saveMetin2Resolution = saveMetin2Resolution
window.saveMetin2Fullscreen = saveMetin2Fullscreen
window.onMetin2TabSelected = onMetin2TabSelected
window.openGameFolder = openGameFolder
window.openConfigFolder = openConfigFolder
window.initializeAccountActionButtons = initializeAccountActionButtons
window.addAccount = addAccount
window.initializeGraphicsQualitySelector = initializeGraphicsQualitySelector
window.saveGraphicsQualitySettings = saveGraphicsQualitySettings
window.loadGraphicsQualityFromConfig = loadGraphicsQualityFromConfig

// Fonction de test pour déboguer
window.testMetin2Dropdown = function() {
    console.log('=== TEST DROPDOWN METIN2 ===')
    const selected = document.getElementById('metin2ResolutionSelected')
    const options = document.getElementById('metin2ResolutionOptions')
    const container = selected ? selected.parentElement : null
    
    console.log('Selected element:', selected)
    console.log('Selected innerHTML:', selected ? selected.innerHTML : 'N/A')
    console.log('Options element:', options)
    console.log('Container:', container)
    console.log('Container classes:', container ? container.className : 'N/A')
    
    if (options) {
        const optionsList = options.querySelectorAll('div[value]')
        console.log('Options trouvées:', optionsList.length)
        optionsList.forEach((opt, i) => {
            console.log(`Option ${i}:`, opt.getAttribute('value'), opt.textContent, opt.hasAttribute('selected') ? '[SELECTED]' : '')
            
            // Test de clic programmatique
            if (i === 0) {
                console.log('Test de clic sur la première option...')
                opt.click()
            }
        })
    }
    
    // Test de re-initialisation
...
// Fonction pour forcer la mise à jour du texte
window.forceUpdateResolutionText = function(text) {
    const selected = document.getElementById('metin2ResolutionSelected')
    if (selected) {
        console.log('Mise à jour forcée du texte:', text)
        selected.innerHTML = text
        console.log('Nouveau texte:', selected.innerHTML)
    }
}